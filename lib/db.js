...
const getRefOfStr = (str) => {
    ...
    if (offset === -1) {
        ...
        const at = db.nameCache;
        db.nameCache = grow(db.nameCache, buffer.byteLength);
        db.nameCache.set(at, buffer);
        const refBuf = Buffer.alloc(5);
        refBuf.writeUint32LE(at, 0);
        refBuf.writeUint8(at+4, buffer.byteLength);
        return refBuf;
    }

    const refBuf = Buffer.alloc(5);
    refBuf.writeUint32LE(offset, 0);
    refBuf.writeUint8(offset+4, buffer.byteLength);
    return refBuf;
}

// Takes [addr, len] and returns a "string"
const getStrOfRef = ([byteOffset, byteLength]) => {
    return db.nameCache.subarray(byteOffset, byteLength).toString();
}

const kSizeOfServer = 4 + 4 + 4 + 5 + 1 + 5 + 4 + 4 // size of the struct
const readServer = (addr) => {
    ...
    const {meta} = server;
    let pos = addr;
    server.id = db.data.subarray(pos, Math.min(db.data.indexOf(0, pos), pos += 4)).toString();
...
const writePlayer = (addr, player) => {
    ...
    pos += 5;
    if (needsInit) db.data.writeFloat32LE(player.uptimeRn, pos);
    pos += 4;
    db.data.writeFloat32LE(player.uptimeRn, pos);
    pos += 4;
    db.data.writeFloat32LE(player.score, pos);
    pos += 4;
    return pos;
}
...
const pushServer = (server) => {
    ...
    let pos = addr;
    db.data.set(Buffer.from(id), pos);
    pos += 4;
...
const updateServer = (server) => {
    const data = db.data;
    const { meta, players, id } = server;
    const latestAddress = data.lastIndexOf(Buffer.from(id + '\x00'));
    const latest = readServer(latestAddress);
    if (latest.meta.uptime < meta.uptime) { // if this is a new server on the same id, just push a new server
        pushServer(server);
    } else {
        let pos = latestAddress;
        pos += 4;
        let newPollCount = latest.pollCount + 1;
        db.data.writeFloatLE((latest.meta.mspt * pollCount + meta.mspt) / newPollCount, pos);
        pos += 4;
        db.data.writeFloatLE(meta.uptime, pos);
        pos += 4;
        pos += 5;
        ...
        db.grow((playersKnown.length - curPlayers) * kSizeOfPlayer);
        for (; i < playersKnown.length; ++i) pos = writePlayer(pos, playersKnown[i]);
    }
}

db.update = (servers) => {
    db.data = readFileSync(OBJ_FILE, 'binary');
